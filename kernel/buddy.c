...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  pair alloc:");
    bd_print_vector(bd_sizes[k].pair_alloc, NBLK(k) / 2 + NBLK(k) % 2);
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_inverse(bd_sizes[k].pair_alloc, blk_index(k, p) / 2);
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_inverse(bd_sizes[k - 1].pair_alloc, blk_index(k - 1, p) / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    int is_buddy_free = bit_isset(bd_sizes[k].pair_alloc, buddy / 2);
    bit_inverse(bd_sizes[k].pair_alloc, bi / 2);           // free p at size k
    if (!is_buddy_free) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      bit_inverse(bd_sizes[k].pair_alloc, bi / 2);
    }
  }
}
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    pair_sz = sizeof(char) * ROUNDUP(NBLK(k) / 2 + NBLK(k) % 2, 8) / 8;
    bd_sizes[k].pair_alloc = p;
    memset(bd_sizes[k].pair_alloc, 0, pair_sz);
    p += pair_sz;
  }
